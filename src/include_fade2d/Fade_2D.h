...
/** \brief Fade_2D is the Delaunay triangulation class
*
...
/** \brief Copy constructor, copy also zones
*
 *
*/
 ...
 * @param [in] vSaveZones is used specify zones that shall additionally
 * be saved
 *
...
/** \brief Set the number CPU cores for multithreading (deprecated)
 *
 * @warning This method is deprecated in favor of the free function
 * int setGlobalNumCPU(). It is kept for backwards compatibility
 * and it internally forwards the call to setGlobalNumCPU().
 */
 ...
 * compute a 100% perfect Delaunay triangulation. However, the quality
 * difference to using double precision arithmetic is hardly
 * noticeable. It is often rather relevant in scientific applications
...
* @param bWithConstraints specifies if constraint segments shall be shown (default: true)
*
* @note When the last elements have been drawn, the postscript file must be finalized with Visualizer2::writeFile().
*
*/
...
*
* \note \p vPoints must not be vertices of any ConstraintGraph2 or
* ConstraintSegment2 object. In this case the vertex is not removed.
*/
...
* @return the vertex pointer of \p or NULL if \p is not a vertex of the triangulation
*/
...
#if GEOM_PSEUDO3D==GEOM_TRUE
/** \brief Get a peeled zone
 * The getPeeledZone() function returns a zone that includes all triangles of \p dt except
 * for the almost vertical ones typically found at the boundary of a 2.5D triangulation.
 * More precisely, this function iteratively identifies and removes boundary triangles
 * whose normal vector encloses a larger angle with the vector (0,0,1) than \p angleThreshold
 * degrees. If no triangles remain, the method returns NULL.
 *
 * @param [in] angleThreshold is the maximum angle between the normal vector of any kept
 * triangle and the vector (0,0,1). Use for example 89.9 degrees.
 * @param [in] bAvoidSplit prevents the zone from breaking apart i.e., if bAvoidSplit=true
 * and removing a triangle would split the zone in two independent connected components
 * then the triangle is not removed.
 * @return the peeled Zone2* or NULL if no triangles remain.
 *
 * \see Zone2::peelOffIf()
 */
...
*
* \warning Using this older version of the method is only recommended if you definitely know
* that your input triangulation is 100% correct. If there is any chance that the input points
* have been rounded (e.g., due to conversion from 64 to 32 bit precision or simply because
* they have been parsed from an ASCII file), then please use the robust version
* importTriangles_robust() instead.
*
* \param vPoints Contains the input triangulation (3 subsequent points per triangle).
* \param bReorientIfNeeded specifies whether the orientations of the point triples
* shall be checked and corrected. If you know that the point triples are certainly
* oriented in counterclockwise order, then the orientation test can be skipped.
* \param bCreateExtendedBoundingBox can be used to insert the 4 corners of an
* extended bounding box as dummy points. This is convenient in some cases. Use
...
*
* This method imports triangles into an empty Fade object. In contrast
* to the classic importTriangles() method, this robust version works
* also in presence of errors in the input data, and corrects overlapping 
* triangles automatically. 
*
* \param vPoints contains the input triangulation (3 subsequent points per triangle).
* \return a pointer to a Zone2 object or NULL if the input data is invalid.
*
* @note This robust version is very fast, and due to the additional
* checks, it is also very safe. Thus, it is highly recommended to use
* it always rather than the classic importTriangles() method. Only if
* your data is huge and definitely correct, then importTriangles()
* may save some time.
*/
...
*/
	...
	/// @private Deprecated, kept for backwards compatibility
 ...
 *
 * @param [in] vSegments specifies the input polygon
 ...
 *
 * Properties: The input polygon (\p vSegments) is not required to have specific height values;
 * the z-coordinates are calculated automatically. The input polygon must be simple, meaning
 * it must not intersect itself, and each vertex must belong to exactly two segments.
 * Alternatively, the polygon may self-intersect at vertices, but in that case, all segments
 * must be oriented counterclockwise. The input polygon is automatically trimmed when it lies
 * outside the convex hull of the triangulation. Inserting intersection points would normally
 * cause existing edges in the triangulation to flip, but this can be avoided by setting
 * bProtectEdges=true. In this case, certain edges of the triangulation will become constraint
 * edges.
 *
 */
 ...
 * \if SECTION_FADE25D
 * @param [in] offTolerance is used to prevent truncation of segments that are slightly outside the convex hull
 * which might be the case if your input segments are subject to tiny rounding errors.
 * \endif
 *
